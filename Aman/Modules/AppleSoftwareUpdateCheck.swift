...
class AppleSoftwareUpdateCheck: SystemCheck {
    init() {
        super.init(
    ...
    override func check() {
        ...
        do {
            ...
            let trimmed = output.replacingOccurrences(of: "\"", with: "")
            ...
            if let lastUpdateDate = legacyFormatter.date(from: trimmed) ?? isoFormatter.date(from: trimmed) ?? ISO8601DateFormatter().date(from: trimmed) {
